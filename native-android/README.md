...
Phase 5 checkpoint 4 includes native Firebase authentication, compatibility
...
calendar, smoother animated date/filter controls, a combined class entry screen
...
a reports/export preview screen, multi-select institute scope filters, and Android
share-sheet text export, visible custom report date controls, and native PDF
report generation shared through Android's share sheet. Native PDF reports now
follow the teacher web app export structure more closely with a `ClassLog`
header, summary cards, institute/class sections, and entry tables, and PDF
exports are saved under app files with stable filenames plus progress,
completion, and failure states. Production native builds remain read-only, and
the teacher/admin web applications continue to use the existing Firestore model.

See `../docs/native-android-phase-5-checkpoint-4.md` for the latest durable
native PDF export behavior, verification, and rollback details. See
`../docs/native-android-phase-5-checkpoint-3.md` for web-style native PDF report
behavior, see
`../docs/native-android-phase-5-checkpoint-2.md` for PDF report export and
custom range behavior, see
...
see
`../docs/native-android-phase-4-checkpoint-5.md` for recoverable
delete/restore behavior, see
