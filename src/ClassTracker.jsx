...
function Calendar({ color, notes, onSelectDate, selectedDate }) {
  ...
  return (
    <div style={{ background:"#fff", borderRadius:16, overflow:"hidden", boxShadow:"0 2px 8px rgba(0,0,0,0.06)", border:"1.5px solid #F0F0F0" }}>
      <div style={{ background:color.bg, padding:"12px 16px", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
        <button onClick={prevMonth} style={{ background:"rgba(255,255,255,0.25)", border:"none", borderRadius:7, width:28, height:28, cursor:"pointer", color:"#fff", fontSize:16, display:"flex", alignItems:"center", justifyContent:"center" }}>‹</button>
        <div style={{ textAlign:"center" }}>
          <div style={{ color:"#fff", fontSize:15, fontWeight:700 }}>{MONTHS[calMonth]}</div>
          <div style={{ color:"rgba(255,255,255,0.75)", fontSize:11, fontFamily:"monospace" }}>{calYear}</div>
        </div>
        <button onClick={nextMonth} style={{ background:"rgba(255,255,255,0.25)", border:"none", borderRadius:7, width:28, height:28, cursor:"pointer", color:"#fff", fontSize:16, display:"flex", alignItems:"center", justifyContent:"center" }}>›</button>
      </div>
      <div style={{ display:"grid", gridTemplateColumns:"repeat(7,1fr)", padding:"8px 8px 2px" }}>
        {DAYS.map(d=><div key={d} style={{ textAlign:"center", fontSize:10, color:"#bbb", fontFamily:"monospace", padding:"2px 0" }}>{d}</div>)}
      </div>
        ...
        {cells.map((day,i) => {
          ...
          return (
            <div key={dk} onClick={()=>onSelectDate(dk)}
              style={{ position:"relative", aspectRatio:"1", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", borderRadius:8, cursor:"pointer", background:isSelected?color.bg:isToday?color.light:"transparent", border:isSelected||isToday?`2px solid ${color.bg}`:"2px solid transparent", transition:"all 0.12s" }}
              ...
              <span style={{ fontSize:12, fontWeight:isToday||isSelected?700:400, color:isSelected?"#fff":isToday?color.text:"#333", lineHeight:1 }}>{day}</span>
              {firstTime && <span style={{ fontSize:7, color:isSelected?"rgba(255,255,255,0.85)":color.text, fontFamily:"monospace", lineHeight:1.2 }}>{fmt(firstTime)}</span>}
              {count>0 && (
                <div style={{ position:"absolute", bottom:2, display:"flex", gap:1.5 }}>
                  {count<=3 ? Array.from({length:count}).map((_,di)=><div key={di} style={{ width:3, height:3, borderRadius:"50%", background:isSelected?"rgba(255,255,255,0.8)":color.bg }} />) : <div style={{ fontSize:8, fontFamily:"monospace", color:isSelected?"rgba(255,255,255,0.9)":color.text, fontWeight:700 }}>{count}</div>}
                </div>
              )}
            </div>
          );
      ...
      <div style={{ padding:"0 12px 10px", display:"flex", gap:12, alignItems:"center" }}>
        <div style={{ display:"flex", alignItems:"center", gap:4 }}><div style={{ width:8, height:8, borderRadius:"50%", background:color.bg }} /><span style={{ fontSize:10, color:"#aaa", fontFamily:"monospace" }}>entries</span></div>
        <div style={{ display:"flex", alignItems:"center", gap:4 }}><div style={{ width:8, height:8, borderRadius:2, background:color.light, border:`1.5px solid ${color.bg}` }} /><span style={{ fontSize:10, color:"#aaa", fontFamily:"monospace" }}>today</span></div>
      </div>
    </div>
  );
}
...
export default function ClassTracker() {
  ...
  const [newClass, setNewClass] = useState({ name:"", subject:"", teacher:"", timeStart:"", timeEnd:"" });
  ...
  const addClass = () => {
    ...
    setData(d=>({ ...d, classes:[...d.classes,{ id, name:newClass.name.trim(), subject:newClass.subject.trim(), teacher:newClass.teacher.trim(), timeStart:newClass.timeStart, timeEnd:newClass.timeEnd, colorIdx:d.classes.length%COLORS.length, created:Date.now() }], notes:{...d.notes,[id]:{}}}));
    setNewClass({ name:"", subject:"", teacher:"", timeStart:"", timeEnd:"" });
  };
    ...
    <div style={{ minHeight:"100vh", background:"#F7F5F0", fontFamily:"Georgia,serif" }}>
      ...
      <div style={{ maxWidth:640, margin:"0 auto", padding:"32px 20px" }}>
              ...
              <div key={cls.id} onClick={()=>{ setActiveClass(cls); setView("class"); setSelectedDate(todayKey()); setSearch(""); }}
                ...
                <div style={{ flex:1, minWidth:0 }}>
                  ...
                  <div style={{ display:"flex", gap:10, marginTop:3, flexWrap:"wrap" }}>
                    ...
                    {cls.timeStart && <span style={{ fontSize:12, color:"#888" }}>🕐 {formatPeriod(cls.timeStart, cls.timeEnd)}</span>}
                  </div>
                </div>
        ...
        <div style={{ marginTop:24, background:"#fff", borderRadius:16, padding:20, border:"1.5px dashed #D0D0D0" }}>
          ...
          <label style={{ fontSize:11, color:"#aaa", fontFamily:"monospace", letterSpacing:1, display:"block", marginBottom:5, marginTop:4 }}>CLASS PERIOD</label>
            ...
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
              <div>
                ...
                <div style={{ display:"flex", flexWrap:"wrap", gap:12, marginTop:5 }}>
                  ...
                  {activeClass.timeStart && <span style={{ color:"rgba(255,255,255,0.85)", fontSize:13 }}>🕐 {formatPeriod(activeClass.timeStart, activeClass.timeEnd)}</span>}
                </div>
              </div>
        ...
        <div style={{ maxWidth:680, margin:"0 auto", padding:"24px 20px" }}>
          ...
          <div style={{ marginTop:28 }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:14 }}>
              ...
              <button onClick={()=>{ setNewNote({title:"",body:"",tag:"note",timeStart:"",timeEnd:""}); setView("addNote"); }} style={{ background:color.bg, color:"#fff", border:"none", borderRadius:10, padding:"9px 18px", fontSize:13, cursor:"pointer", fontFamily:"monospace", fontWeight:600 }}>+ Add Entry</button>
            </div>
                  ...
                  <div key={note.id} style={{ background:"#fff", borderRadius:14, padding:"16px 18px", border:"1.5px solid #EFEFEF", boxShadow:"0 1px 4px rgba(0,0,0,0.05)" }}>
                    <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:note.body?10:0 }}>
                      <div style={{ flex:1 }}>
                        ...
                        {note.timeStart && <span style={{ fontSize:11, color:"#888", fontFamily:"monospace", marginRight:8 }}>🕐 {formatPeriod(note.timeStart, note.timeEnd)}</span>}
    ...
    return (
      <div style={{ minHeight:"100vh", background:"#F7F5F0", fontFamily:"Georgia,serif" }}>
        ...
        <div style={{ maxWidth:640, margin:"0 auto", padding:"28px 20px" }}>
          ...
          <label style={{ fontSize:11, color:"#aaa", fontFamily:"monospace", letterSpacing:1, display:"block", marginBottom:5 }}>CLASS PERIOD (optional)</label>
          <div style={{ display:"flex", gap:8, alignItems:"center", marginBottom:10 }}>
