...
import { canUseGooglePopupAuth, getTeacherAppUrl, isNativeApp } from "./platform";
...
export default function AdminAuth({ onVerified, currentUser = null }) {
  ...
  const [mode,    setMode]    = useState(nativeApp ? "email" : "choose");
  ...
  return (
    <div style={{minHeight:"100svh",width:"100%",overflowX:"hidden",background:G.navy,fontFamily:G.sans,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"20px 16px",position:"relative",overflow:"hidden"}}>
      ...
      <div style={{width:"100%",maxWidth:400,background:"rgba(255,255,255,0.06)",borderRadius:20,padding:"22px 16px",border:"1px solid rgba(255,255,255,0.1)",boxShadow:"0 24px 64px rgba(0,0,0,0.3)"}}>
        ...
        {nativeApp && (
          <div style={{marginTop:14,background:"rgba(59,130,246,0.12)",border:"1px solid rgba(59,130,246,0.25)",borderRadius:10,padding:"10px 14px",fontSize:14,color:"rgba(255,255,255,0.75)",lineHeight:1.5}}>
            Mobile app note: existing admins can sign in with email today. Invite-link activation and native Google sign-in still need deep-link and OAuth wiring.
          </div>
        )}
      </div>
