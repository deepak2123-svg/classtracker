...
const G = {
  ...
  sans:"'Plus Jakarta Sans',sans-serif", display:"'Syne',sans-serif",
  mono:"'JetBrains Mono',monospace",
};

const inp = {
  width:"100%", padding:"12px 14px", borderRadius:10,
  border:`1.5px solid ${G.border}`, fontSize:16,
    ...
    <div style={{minHeight:"100vh",background:G.bg,fontFamily:G.sans,display:"flex",alignItems:"center",justifyContent:"center",padding:"16px 14px"}}>
      <div style={{width:"100%",maxWidth:420}}>
        ...
        <div style={{textAlign:"center",marginBottom:28}}>
          ...
          <p style={{margin:"5px 0 0",color:G.textL,fontSize:13}}>Your academic planner</p>
        </div>
        ...
        <div style={{background:G.surface,borderRadius:18,padding:"22px 16px",boxShadow:"0 4px 24px rgba(14,31,24,0.08)",border:`1px solid ${G.border}`}}>
          ...
          <div style={{display:"flex",background:G.bg,borderRadius:10,padding:3,marginBottom:22}}>
            {["login","signup"].map(t=>(
              <button key={t} onClick={()=>{setTab(t);setError("");}}
                style={{flex:1,padding:"9px 0",border:"none",borderRadius:8,fontSize:12,fontFamily:G.mono,letterSpacing:1,cursor:"pointer",transition:"all 0.15s",
          ...
          <button onClick={handleGoogle} disabled={loading}
            style={{width:"100%",padding:"12px 0",border:`1.5px solid ${G.border}`,borderRadius:10,background:G.surface,fontSize:14,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:9,marginBottom:18,fontFamily:G.sans,color:G.textM,fontWeight:500,transition:"border-color 0.15s",minHeight:48}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:18}}>
            ...
            <span style={{fontSize:11,color:G.textL,fontFamily:G.mono}}>or</span>
            <div style={{flex:1,height:1,background:G.border}}/>
          </div>

          {tab==="signup"&&(
            <>
              <label style={{fontSize:11,color:G.textL,fontFamily:G.mono,letterSpacing:0.5,display:"block",marginBottom:5,textTransform:"uppercase"}}>Your name</label>
              <input value={name} onChange={e=>setName(e.target.value)} placeholder="e.g. Ramsingh Yadav" style={inp}/>
            </>
          )}

          <label style={{fontSize:11,color:G.textL,fontFamily:G.mono,letterSpacing:0.5,display:"block",marginBottom:5,textTransform:"uppercase"}}>Email address</label>
          ...
          <label style={{fontSize:11,color:G.textL,fontFamily:G.mono,letterSpacing:0.5,display:"block",marginBottom:5,textTransform:"uppercase"}}>Password</label>
          ...
          {error&&<div style={{background:G.redL,color:G.red,borderRadius:9,padding:"9px 12px",fontSize:13,marginBottom:16,fontFamily:G.sans}}>{error}</div>}

          <button onClick={handleSubmit} disabled={loading}
            style={{width:"100%",padding:"13px",borderRadius:10,border:"none",background:loading?"#B0C4B8":G.forest,color:"#fff",fontSize:14,cursor:loading?"not-allowed":"pointer",fontFamily:G.sans,fontWeight:600,letterSpacing:0.2,transition:"background 0.15s",minHeight:48}}>
            {loading?"Please wait…":tab==="login"?"Log In":"Create Account"}
          </button>
        </div>

        <p style={{textAlign:"center",fontSize:12,color:G.textL,marginTop:16,fontFamily:G.sans}}>Your data is saved securely to your account.</p>
      </div>
    </div>
  );
}
