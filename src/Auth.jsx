...
import { canUseGooglePopupAuth, hasNativeGoogleClientId, isNativeApp } from "./platform";
...
export default function Auth() {
  ...
  const hasNativeGoogleConfig = hasNativeGoogleClientId();
  const showGoogleAuth = canUseGooglePopupAuth() || nativeApp;
    ...
    <div style={{minHeight:"100svh",width:"100%",overflowX:"hidden",background:G.bg,fontFamily:G.sans,display:"flex",alignItems:"center",justifyContent:"center",padding:"16px 14px"}}>
      <div style={{width:"100%",maxWidth:420}}>
        ...
        <div style={{background:G.surface,borderRadius:18,padding:"22px 16px",boxShadow:"0 4px 24px rgba(14,31,24,0.08)",border:`1px solid ${G.border}`}}>
          ...
          {nativeApp && (
            <div style={{background:G.greenL,color:G.green,borderRadius:10,padding:"10px 12px",fontSize:14,marginBottom:18,lineHeight:1.5}}>
              {hasNativeGoogleConfig
                ? "Android note: Google sign-in is enabled in the app shell. If it fails, the remaining setup is usually SHA-1 or Firebase console configuration."
                : "Android note: Google sign-in is wired in code, but this build still needs VITE_GOOGLE_WEB_CLIENT_ID before the button can work."}
            </div>
          )}
