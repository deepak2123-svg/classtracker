...
function GradeGroupModal({ inst, group, onSave, onClose }) {
  ...
  // Track the visual viewport (shrinks when keyboard opens on mobile)
  const [vp, setVp] = React.useState(() => {
    const vv = window.visualViewport;
    return vv ? { top: vv.offsetTop, left: vv.offsetLeft, width: vv.width, height: vv.height } : null;
  });

  React.useEffect(() => {
    // Lock body scroll so the page doesn't scroll under the modal
    const prev = document.body.style.overflow;
    document.body.style.overflow = "hidden";

    const vv = window.visualViewport;
    if (!vv) return () => { document.body.style.overflow = prev; };
