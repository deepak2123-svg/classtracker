...
function AdminPanelInner({user}){
  ...
  const handlePromote=async(uid)=>{
    adminConfirmDialog("Promote to Admin? They will see all data.","Promote",async()=>{
      try {
        ...
        showAdminToast("Admin access granted.");
      } catch (e) {
  ...
  const handleDemote=async(uid)=>{
    adminConfirmDialog("Remove admin access?","Remove Admin",async()=>{
      try {
        ...
        showAdminToast("Admin access removed.");
      } catch (e) {
        showAdminToast("Failed: " + e.message);
      }
    });
  ...
  const AdminConfirmModal = () => adminConfirm ? (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.55)",zIndex:9998,display:"flex",alignItems:"center",justifyContent:"center",padding:20,backdropFilter:"blur(4px)"}}>
